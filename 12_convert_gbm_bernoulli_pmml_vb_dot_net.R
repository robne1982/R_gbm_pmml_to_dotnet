...
file.remove("vb_dot_net_model_titanic.txt")
fileConn <- file("vb_dot_net_model_titanic.txt", 'a')

smpModelXML <- xmlToList(xmlParse('titanicmdl.pmml'))
...
miningModelTarget <- smpModelXML$MiningModel$Segmentation[[1]]$MiningModel$Targets
miningModelSegmentation <- smpModelXML$MiningModel$Segmentation[[1]]$MiningModel$Segmentation

length(miningModelSegmentation)

rescaleConstant <- miningModelTarget$Target[1]
